...
Convert from root respiration data reported in George et al (where O_2
...
Q_{10} = 2.075 from George 2003, and the measurement temperature of
...
values are different, it is not likely that Q_{10} or the atmospheric
...
|:---------|:------|:------------------------|:-----------|
| Vcmax | \mu mol CO_2 m^{2} s^{-1} | 44 (12, 125) | maximum rubisco carboxylation capacity |
...
| GS | | | stomatal conductance (= gs_{\textrm{max}} |
...
| R_{\textrm{dark}} | | \mu mol CO_2 m^{-2} s^{-1} | dark respiration |
...
| root respiration rate* | CO_2 kg^{-1} fine roots s^{-1} | 1--100 | rate of fine root respiration at reference soil temperature |
...
| lb | kg | Y=X\times 2.2 | |
| mm/s | \mu mol CO_2 m^{2} s^{-1} | Y=X\times 0.04 | |
| m^2 | ha | Y = X/10^6 | |
...
| mg CO_2 g^{-1} h^{-1} | \mu mol kg^{-1} s^{-1} | Y = X\times 6.31 | used for root\_respiration\_rate |
